        ...
        child: Column(
          ...
          children: [
            ...
            if (title.contains('Authentication'))
              ElevatedButton.icon(
                onPressed: () async {
                  await Provider.of<AppState>(context, listen: false).mockLogin();
                  if (context.mounted) {
...
class AppRouter {
  static Route<dynamic> onGenerateRoute(RouteSettings settings) {
    return MaterialPageRoute(
      settings: settings,
      builder: (context) => _AuthGuard(settings: settings, child: _getScreen(settings)),
    );
  }
...
class _AuthGuard extends StatelessWidget {
  ...
  const _AuthGuard({required this.settings, required this.child, Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final routeName = settings.name;

    if (routeName == SplashScreen.routeName || routeName == AuthScreen.routeName) {
      return child;
    }
